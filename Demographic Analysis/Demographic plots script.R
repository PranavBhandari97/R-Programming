...
#Importing qplot to create visual
#install.packages('ggplot2')
library(ggplot2)
plot = qplot(data=data,  x = Birth.rate,y = Internet.users, colour = Income.Group, 
             size = I(4), shape = I(19), alpha = I(0.7))
plot + xlab("Birth Rate") + ylab("Internet Users") +
...
df = data.frame(Country = Countries,Code = Codes, Region = Regions)
...
#Merging the Dataset and DataFrame
head(data)
head(df)
final_dataset = merge(data,df, by.x = 'Country.Code', by.y = 'Code')
head(final_dataset)

#Dropping the duplicate column
final_dataset$Country = NULL
head(final_dataset)

#Creating the visual
plot2 = qplot(data=final_dataset,  x = Birth.rate,y = Internet.users, 
              colour = Region, size = I(4), shape = I(19), alpha = I(0.7))
plot2 + xlab("Birth Rate") + ylab("Internet Users") +
  ggtitle("Internet Users vs Birth Rate") + 
  theme(axis.title.x = element_text(color = 'Blue',size=20),
        axis.title.y = element_text(color = 'Blue',size=20),
        axis.text.x = element_text(size=15),
